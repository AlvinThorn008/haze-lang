...
impl<'s, 'b> Parser<'s, 'b> {
    pub(crate) fn if_expr(&mut self) -> Result<Node<'s, 'b>, ParsingError> {
        ...
        let condition = self.expr_with_restrictions(Restrictions::BLOCK)?;
        node.add(condition);
        ...
        match self.peek() {
            Some(tok) if tok.tag == Tag::If => alternate = self.if_expr()?,
            Some(tok) if tok.tag == Tag::LBrace => alternate = self.block_expr()?,
            Some(tok) => {
